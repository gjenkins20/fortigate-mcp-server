...
class FortiGateMCPHTTPServer:
    ...
    def __init__(
        ...
        if not FASTMCP_AVAILABLE:
            raise RuntimeError(
                "FastMCP is not available. Please install fastmcp package."
            )
            ...
            for device_id in devices:
                try:
                    ...
                    if success:
                        ...
                        self.logger.warning(
                            f"Connection test failed for device: {device_id}"
                        )
                except Exception as e:
                    self.logger.error(
                        f"Connection test error for device {device_id}: {e}"
                    )
    ...
    def _setup_tools(self) -> None:
        ...
        def create_firewall_policy(
            device_id: str, policy_data: dict, vdom: Optional[str] = None
        ):
        ...
        def get_firewall_policy_detail(
            device_id: str, policy_id: str, vdom: Optional[str] = None
        ):
        ...
        def create_address_object(
            ...
            return self.network_tools.create_address_object(
                device_id, name, address_type, address, vdom
            )
        ...
        def create_static_route(
            ...
            return self.routing_tools.create_static_route(
                device_id, dst, gateway, device, vdom
            )
        ...
        def update_static_route(
            device_id: str, route_id: str, route_data: dict, vdom: Optional[str] = None
        ):
        ...
        def delete_static_route(
            device_id: str, route_id: str, vdom: Optional[str] = None
        ):
        ...
        def update_virtual_ip(
            device_id: str, name: str, vip_data: dict, vdom: Optional[str] = None
        ):
        ...
        def get_virtual_ip_detail(
            device_id: str, name: str, vdom: Optional[str] = None
        ):
        ...
        def test_connection():
            try:
                ...
                return self._format_response(
                    {"devices": connection_results, "total_devices": len(devices)},
                    "test_connection",
                )
            except Exception as e:
                return self._format_response(
                    {"success": False, "error": str(e)}, "test_connection"
                )
    ...
    def run(self) -> None:
        ...
        try:
            self.logger.info(
                f"Starting FortiGate MCP HTTP server on "
                f"{self.host}:{self.port}{self.path}"
            )
            self.logger.info(
                f"Registered devices: {len(self.fortigate_manager.devices)}"
            )

            # Run with FastMCP's built-in HTTP transport
            self.mcp.run(
                transport="http", host=self.host, port=self.port, path=self.path
            )
        except Exception as e:
