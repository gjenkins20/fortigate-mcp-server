...
class FortiGateTemplates:
    ...
    @staticmethod
    def firewall_policy_detail(
        policy_data: Dict[str, Any],
        device_id: str,
        address_objects: Optional[Dict[str, Any]] = None,
        service_objects: Optional[Dict[str, Any]] = None,
    ) -> str:
        ...
        lines.extend(
            [
                ...
                f"  Status: {'Active' if policy.get('status') == 'enable' else 'Disabled'}",  # noqa: E501
        ...
        src_intf_names = [
            ...
            for intf in src_intf
        ]
        dst_intf_names = [
            intf.get("name", "unknown") if isinstance(intf, dict) else str(intf)
            for intf in dst_intf
        ]
            ...
            for src_name in src_names:
                if src_name in addr_dict:
                    ...
                    if addr.get("subnet"):
                        ...
                        lines.append(
                            f"    {src_name}: {addr['start-ip']} - {addr['end-ip']}"
                        )
                    elif addr.get("fqdn"):
            ...
            for dst_name in dst_names:
                if dst_name in addr_dict:
                    ...
                    if addr.get("subnet"):
                        ...
                        lines.append(
                            f"    {dst_name}: {addr['start-ip']} - {addr['end-ip']}"
                        )
                    elif addr.get("fqdn"):
        ...
        lines.extend(
            [
                ...
                f"  Log Traffic: {'Yes' if policy.get('logtraffic') == 'all' else 'No'}",  # noqa: E501
        ...
        schedule_name = (
            ...
            else str(schedule[0]) if schedule else "always"
        )
        lines.append(f"  Schedule: {schedule_name}")

        # Comments
        if policy.get("comments"):
            lines.extend(["", "Comments", f"  {policy['comments']}"])
        ...
        lines.extend(
            [
                ...
                f"  Internet Service: {'Yes' if policy.get('internet-service') == 'enable' else 'No'}",  # noqa: E501
                f"  Application Control: {'Yes' if policy.get('application-list') else 'No'}",  # noqa: E501
    ...
    def virtual_ip_detail(vip_data: Dict[str, Any]) -> str:
        ...
        if "results" in vip_data and vip_data["results"]:
            vip = (
                vip_data["results"][0]
                if isinstance(vip_data["results"], list)
                else vip_data["results"]
            )
    ...
    def operation_result(
        ...
        if success and details:
            lines.extend(["Details:", f"  {details}", ""])
